...
export function applyTransforms(rewriterState: TransformResult, transforms: CodeTransform[]) {
  const { create, getOwnPropertyDescriptors } = Object
  const { apply } = Reflect
  const uncurryThis = <T extends (this: V, ...args: any) => any, V>(fn: T) =>
    ((thisArg: ThisParameterType<T>, ...args: Parameters<T>): ReturnType<T> => apply(fn, thisArg, args))
  ...
  rewriterState = arrayReduce(
    transforms,
    (rs: any, transform: any) => (transform.rewrite ? transform.rewrite(rs) : rs),
    rewriterState
  )
